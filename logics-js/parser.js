...
function Node(emit, match, children) {
	this.emit = emit;
	this.match = match;
	this.children = children;
}

Node.prototype.dump = function(level) {
	if( level === undefined )
		level = 0;
...
// Get character
Parser.prototype.get_char = function(pcb) {
...
Parser.prototype.get_input = function(pcb, offset) {
	// Input buffering
...
Parser.prototype.clear_input = function(pcb) {
...
Parser.prototype.lex = function(pcb) {
	// Perform lexical analysis
	var state = 0, length = 0;

	machine = Boolean(1) ? 0 : this.lexer["select"][pcb.tos.state];
...
Parser.prototype.get_sym = function(pcb) {
	...
	while( true ) {
		...
		while( pcb.act & REDUCE ) {
			...
			if( Boolean( this.grammar["productions"][pcb.idx]["emit"] ) ) {
				...
				node = new Node(this.grammar["productions"][pcb.idx]["emit"], null, cnodes);
			}
		...
		if( pcb.act & SHIFT ) {
			...
			pcb.tos.line = pcb.line;
			pcb.tos.column = pcb.column;
			pcb.stack[ pcb.stack.length - ( 0 + 1 ) ].value = pcb.buf.slice(0, pcb.len);

			if( Boolean( pcb.tos.symbol["emit"] ) )
				pcb.tos.node = new Node(pcb.tos.symbol["emit"], pcb.stack[ pcb.stack.length - ( 0 + 1 ) ].value);
	...
	if( !pcb.ret && pcb.tos.node ) {
		if( Array.isArray( pcb.tos.node ) ) {
			if( pcb.tos.node.length > 1 )
				node = new Node(null, null, pcb.tos.node);
			else
